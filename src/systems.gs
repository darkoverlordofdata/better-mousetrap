...
def inputSystem(game:Game, ref entity:Entity*)
    if entity.active.active && entity.actor.isA == Actor.PLAYER
...
def soundSystem(game:Game, ref entity:Entity)
    if entity.active.active && entity.sound != null
...
def physicsSystem(game:Game, ref entity:Entity*)
    if entity.active.active && entity.velocity != null
...
def expireSystem(game:Game, ref entity:Entity*)
    if entity.active.active && entity.expires != null
...
def tweenSystem(game:Game, ref entity:Entity*)
    if entity.active.active && entity.tween != null
...
def collisionSystem(game:Game, ref entity:Entity*)
    if entity.active.active && entity.style.isA == Category.ENEMY
        for var bullet in game.entity
            if bullet.active.active && bullet.style.isA == Category.BULLET
                if entity.bounds.is_intersecting(bullet.bounds)
                    if entity.active.active && bullet.active.active
